...
import styles from "../../../styles/dashboard/message/page.module.css";
...
export default function Message() {
    ...
    const allChats = async () => {
        try {
            const response = await fetch("http://localhost:9876/allChats", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const filteredChatRequest = chatRequest.filter(r =>
        r.senderId?.username?.toLowerCase().includes(search)
    );

    const filteredSendList = sendList.filter(r =>
        r.userProfileId?.username?.toLowerCase().includes(search)
    );

    const filteredReceiveList = receiveList.filter(r =>
        r.ownerProfileId?.username?.toLowerCase().includes(search)
    );
